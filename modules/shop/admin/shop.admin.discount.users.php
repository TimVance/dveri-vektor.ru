...
/**
 * Shop_admin_discount_users
 */
class Shop_admin_discount_users extends Frame_admin
{
	/**
	...
	*/
	 ...
	 */
	// public $where = " AND (no_auth='1' OR role_id>0 OR person='1' OR threshold_cumulative>0) AND (threshold=0 AND threshold_goods=0 AND threshold_count=0 AND delivery_id=0 AND payment_id=0)";
	public $where = " AND `variable`='users'";
	...
	public $variables = array (
		'main' => array (
			...
			'threshold_combine' => array(
				...
				'help' => 'Скидка будет применяться дополнительно к текущим скидкам.',
				'select' => array(
					"1" => "Да",
					"0" => "Нет",
				),
			...
			'role_id' => array(
				'type' => 'select',
				'name' => 'Скидка только для авторизованных покупателей',
			...
			'person' => array(
				'type' => 'function',
				'name' => 'Только для покупателя (укажите его ID)',
			...
			// 'title1' => array(
			// 	'type' => 'title',
			// 	'name' => 'Создать скидку для пользователей',
			// ),
			'text' => array(
				'type' => 'textarea',
	...
	private function show_content_h1()
	{
		...
		$this->diafan->_admin->name = 'Скидки для пользователей';
	}
	...
	private function show_variable_discount()
	{
		...
		<div class="tabs" id="mode_express">
			...
			<a href="'.$url.'users/'.$site.'" class="tabs__item tabs__item_active">'.$this->diafan->_('Для покупателей').'</a>
			<a href="'.$url.'order/'.$site.'" class="tabs__item">'.$this->diafan->_('На заказы').'</a>
		</div>';
	}
	...
	public function edit_variable_person()
	{
		...
		echo '<div class="param">
			...
			<p><a href="javascript:void(0)" class="param_plus" title="'.$this->diafan->_('Добавить').'"><i class="fa fa-plus-square"></i> '.$this->diafan->_('Добавить купон').'</a></p>
		</div>';
	}

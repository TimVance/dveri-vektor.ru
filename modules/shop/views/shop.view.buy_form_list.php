...
if ($result["row"]["price_arr"])
{
	...
	foreach($result["row"]["price_arr"] as $price)
	{
		...
		{
			echo '<div class="shop_discount">'.($price["role_id"] || $price["person"] ? $this->diafan->_('Ваша скидка') : $this->diafan->_('Скидка')).': <span class="shop_discount_value">'.$price["discount"].' '.$price["discount_currency"].($price["discount_finish"] ? ' ('.$this->diafan->_('до').' '.$price["discount_finish"].')' : '').'</span></div>';
		}

		...
		$this->important_title .=
			'<div class="head-box head-box_warning">
				...
				.' switch="'.($tick_status ? 'on' : 'off').'">
					<i class="fa fa-toggle-on"></i> <span>'.$this->diafan->_('CRON').'</span>
					<img class="spinner hide" src="'.BASE_PATH.'adm/img/loading.gif">
				</a>
			</div>';

		$this->important_title .=
			'<div class="commentary">'.sprintf($this->diafan->_('Бета-версия модуля. Замечания отправляйте в %sТехническую поддержку%s DIAFAN.CMS.'), '<a href="https://user.diafan.ru/support/">', '</a>').'</div><br />';

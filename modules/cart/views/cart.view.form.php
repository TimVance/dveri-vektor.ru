...
echo '
<form method="POST" action="" class="cart_form ajax js_form_order" enctype="multipart/form-data">
<input type="hidden" name="module" value="cart">
		...
		switch ($row["type"])
		{
			...
			case 'datetime':
				...
				echo '<div class="infofield">'.$row["name"].($row["required"] ? '<span style="color:red;">*</span>' : '').':</div>
					...
					.($row["type"] == 'datetime'? 'true' : 'false').'"'.($row["info"] ? ' data-info="'.$row["info"].'"' : '').'>';
				break;
...
if(! empty($result["payments"]))
{
	...
	echo '<div class="payments">'.$this->get('list', 'payment', $result["payments"]).'</div>';
}

...
class Service_express_import extends Diafan
{
	...
	protected function import()
	{
		...
		{
			if(! preg_match('/'. self::COLUMN_NAME .'([0-9]+)/', $name, $result)) continue;
			$fields[] = $name;
		}
	...
	protected function prepare_parent()
	{
		...
		DB::query("ALTER TABLE {%s_category} ADD `import_parent_id` VARCHAR(100) NOT NULL AFTER `import_id`", $this->import["module_name"]);
	}
	...
	private function add_cat_parents($element_id, $parent_id)
	{
		...
		$add_cat_parents = function($element_uid, $parent_uids)
		{
			...
			$e_ids = DB::query_fetch_value(
				...
				$this->import["module_name"], $element_uid, "element_id"
			);
			if(! $e_ids)
			{
				return;
			...
			foreach($e_ids as $e_id)
			{
				foreach($parent_uids as $p_id)
				{
					...
					if(! $id)
					{
						...
						$add_cat_parents($e_id, $parent_uids);
					}
				}
			}
		};
		$add_cat_parents($element_id, $parents);
		return true;
	}

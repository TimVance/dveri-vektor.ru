...
class Order_inc extends Diafan
{
	/**
	...
	public function get($order_id)
	{
		...
		foreach ($rows as $row)
		{
			...
			if(! empty($additional_costs[$row["id"]]))
			{
				foreach($additional_costs[$row["id"]] as $a)
				{
					...
					$row_price["price"] += $a["summ"] / $row["count"]; // $row["price"] += $a["summ"] / $row["count"];
            ...
            $row["old_price"] = '';
			if($row_price["old_price"])
			{
				$row["old_price"] = $this->diafan->_shop->price_format($row_price["old_price"]);
				$result["discount"] = true;
				$row["discount_summ"] = $row_price["old_price"] - ($row["price"] - $row_a_c_summ);
				$row["discount"] = $this->diafan->_shop->price_format($row["discount_summ"]);
			}
			elseif($row_price["price"] != $row["price"])
			{
				$row["old_price"] = $this->diafan->_shop->price_format($row_price["price"]);
				$result["discount"] = true;
				$row["discount_summ"] = $row_price["price"] - $row["price"];
				$row["discount"] = $this->diafan->_shop->price_format($row["discount_summ"]);
			}
		...
		{
			$result["old_summ_goods"] = $this->diafan->_shop->price_format($result["summ_goods"] + $result["discount_summ"]);
		...
		$result["coupon"] = str_replace(',', ', ', $result["coupon"]);
	...
	{
		$this->diafan->_delivery->set_status($order, $status["id"]);
		...
		if($status["send_mail"])
		{
			...
			if($user_mail)
			{
				...
				$this->diafan->_postman->message_add_mail($user_mail, $subject, $message,  $email);
			}
		}
		...
		$this->diafan->_cashregister->receipt_set_status($order, $status["id"]);
	}

	/**
	 * Возврат информацию о плательщике
	 *
	...
	public function details($order_id)
	{
		...
		$a_rows = DB::query_fetch_all("SELECT a.[name], s.summ, a.shop_rel, s.order_goods_id FROM {shop_additional_cost} AS a INNER JOIN {shop_order_additional_cost} AS s ON s.additional_cost_id=a.id AND s.order_id=%d WHERE a.trash='0'", $order_id);
		foreach ($rows as $row)
		{
			$depend = '';
			$params = array();
			...
			foreach($a_rows as $i => $a_row) {
				if(! $a_row["shop_rel"] || $a_row["order_goods_id"] != $row["id"]) {
					continue;
				}
				$row["price"] -= $a_row["summ"];
				$a_rows[$i]["count_summ"] = $row["count_goods"];
			}
		...
		{
			if ($a_row["shop_rel"] && ! empty($a_row["count_summ"])) {
				$a_row["summ"] *= $a_row["count_summ"];
			}
			$order_summ += $a_row["summ"];
			$a_row["summ"] = number_format($a_row["summ"], 2, '.', '');
			$result["additional"][] = $a_row;
		}
	...
	private function send_mail($order_id, $files_list)
	{
		...
		$this->diafan->_postman->message_add_mail(
				...
				$subject,
				$message,
				$this->diafan->configmodules("emailconf", 'shop') ?
						$this->diafan->configmodules("email", 'shop') : EMAIL_CONFIG
		);
		return true;
	}
     ...
     * @return string
     */
    public function get_phone($order_id)
	{

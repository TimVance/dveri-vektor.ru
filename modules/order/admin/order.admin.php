	...
	public $variables = array (
		'main' => array (
			...
			'delivery_info' => array(
				...
				'help' => 'Заполняется бэкендом доставки.',
			),
			'hr3' => 'hr',
			'user_id' => array(
				'type' => 'function',
				...
				if(! empty($additional_costs[$row["good_id"]]))
				{
					foreach($additional_costs[$row["good_id"]] as $a)
					{
						...
						<div class="item__in">
							...
							<div class="num no_important ipad summ_additional_cost">'.$this->format_summ($a["summ"] * $row["count_goods"]).'
							</div>
	...
	public function edit_variable_delivery_id()
	{
		...
		if (! empty($row))
		{
			...
			<div class="item__in">
				...
				<div class="name"><a href="'.BASE_PATH_HREF.'delivery/history/edit'.$row["id"].'/">'.$this->format_summ($row["summ"]).' '.$this->diafan->configmodules("currency", "shop").'</a></div>

...
$position = 1;
echo '<div itemscope="" itemtype="http://schema.org/BreadcrumbList" id="breadcrumbs" class="fs_12 fst-italic mt-2 mx-3">';

if ($this->diafan->_site->id != 1)
{
	echo '<span itemscope="" itemprop="itemListElement" itemtype="http://schema.org/ListItem"><a itemprop="item" class="c_dgrey pe-2" href="'.BASE_PATH_HREF.'"><span itemprop="name">'.$this->diafan->_('Dveri-vektor.ru').'</span><meta itemprop="position" content="'.$position.'"></a> '.$separator.'</span> ';
	$position++;
}
if ($this->diafan->_site->parent_id)
{
	...
	{
		echo '<span itemscope="" itemprop="itemListElement" itemtype="http://schema.org/ListItem"><a itemprop="item" class="c_dgrey px-2" href="'.BASE_PATH_HREF.$row["link"].'"><span itemprop="name">'.$this->diafan->_useradmin->get($row["name"], 'name', $row["id"], 'site', _LANG).'</span><meta itemprop="position" content="'.$position.'"></a> '.$separator.'</span> ';
		$position++;
	}
}
if ($this->diafan->_site->breadcrumb)
{
	foreach ($this->diafan->_site->breadcrumb as $breadcrumb)
	{
		...
		echo '<span itemscope="" itemprop="itemListElement" itemtype="http://schema.org/ListItem"><a itemprop="item" class="c_dgrey px-2" href="'.BASE_PATH_HREF.$breadcrumb["link"].'"><span itemprop="name">'.$breadcrumb["name"].'</span><meta itemprop="position" content="'.$position.'"></a> '.$separator.'</span> ';
		$position++;
	}
}
if($current)
{
	...
	echo '<span itemscope="" itemprop="itemListElement" itemtype="http://schema.org/ListItem"><span itemprop="item"><span itemprop="name">'.$name.'</span><meta itemprop="position" content="'.$position.'"></span> '.$separator.'</span>';
}
echo '</div>';

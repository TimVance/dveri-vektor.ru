...
echo '
<form method="post" action="" class="ajax js_shop_form mx-0 my-2 position-relative px-0 row align-items-center shop_form">
...
if ($result["row"]["price_arr"])
{
	foreach ($result["row"]["price_arr"] as $price)
	{
		...
		echo '<div class="js_shop_param_price shop_param_price shop-item-price col pr-1 text-center"'.$param_code.' price-id-js="'.$price["id"].'">';
			...
			echo '<span class="price c_dblue"><span class="js_shop_price" summ="'.$price["price_no_format"].'" format_price_1="'.$this->diafan->configmodules("format_price_1", "shop").'" format_price_2="'.$this->diafan->configmodules("format_price_2", "shop").'" format_price_3="'.$this->diafan->configmodules("format_price_3", "shop").'">'.$price["price"].'</span>';
	...
	echo '<div class="addict-field d-none">';
...
echo '<div class="js_shop_buy shop_buy col to-cart">';
	...
	{
//		echo '<input type="button" class="button solid btn_buy" value="'.$this->diafan->_('Купить', false).'" action="buy">';
//		echo '<span class="btn_buy"><input type="button" class="button solid" value="'.$this->diafan->_('Купить', false).'" action="buy"></span>';
		echo '<button type="reset" class="button fs_18 px-2 py-2 text-nowrap btn_buy" action="buy"><i class="fa fa-shopping-basket fs_14 me-2" aria-hidden="true"></i>'.$this->diafan->_('В корзину').'</button>';
	}
echo '</div>';

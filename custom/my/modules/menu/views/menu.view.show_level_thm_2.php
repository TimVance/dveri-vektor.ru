...
// начало уровня меню
echo '<ul class="pod_menu dropdown-menu b_dgrey" aria-labelledby="TopHoverMenu">';
foreach ($result["rows"][$result["parent_id"]] as $row)
{
	if ($row["active"])
	{
		// начало пункта меню для текущей страницы
		echo '<li class="active nav-item float-none ">';
	}
	elseif ($row["active_child"])
	{
		// начало пункта меню для активного дочернего пункта
		echo ' <li class="doch_2 active nav-item position-relative dropdown float-none ">';
	}
	elseif ($row["children"])
	{
		// начало пункта меню для элемента -родителя
		echo ' <li class="rod_2 nav-item position-relative dropdown float-none ">';
	}
	else
	{
		// начало любого другого пункта меню
		echo '<li class="drugoy_2 position-relative nav-item float-none d-inline">';
	}
	
	if ($row["active"] && $row["children"]) 
	{
		// начало пункта меню для активного родителя
		echo '<li class="rod_2 active nav-item dropdown float-none ">';
	}
	...
	)
	{
		...
		{
			echo '<a class="nav-link dropdown-item dropdown-toggle py-2 td-none" href="'.BASE_PATH_HREF.$row["link"].'"'.$row["attributes"].'>';
		}
	}
	...
	{
		echo '<i class="fa fa-chevron-down thm_icon2" aria-hidden="true"></i></a>';
	}
	...
	if ($result["show_all_level"] || $row["active_child"] || $row["active"])
	{
		...
		echo $this->get('show_level_thm_3', 'menu', $menu_data);
	}
	if ($row["active"])
	{

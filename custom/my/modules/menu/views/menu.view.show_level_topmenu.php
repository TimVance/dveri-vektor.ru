...
foreach ($result["rows"][$result["parent_id"]] as $row)
{
	...
	elseif ($row["active_child"])
	{
		...
		echo ' <li class="parent active nav-item">';
	}
	elseif ($row["children"])
	{
		// начало пункта меню для элемента -родителя
		echo ' <li class="parent nav-item">';
	}
	else
	{
	...
	)
	{
		if ($row["othurl"])
		{
			echo '<a class="nav-link py-1 c_d_blue text-decoration-none" href="'.$row["othurl"].'"'.$row["attributes"].''
			.(! empty($row["active"]) || ! empty($row["active_child"]) ? ' class="active"' : '')
			.'>';
		}
		else
		{
			echo '<a class="nav-link py-1 c_d_blue text-decoration-none" href="'.BASE_PATH_HREF.$row["link"].'"'.$row["attributes"]
			.(! empty($row["active"]) || ! empty($row["active_child"]) ? ' class="active"' : '')
			.'>';
		}
	}

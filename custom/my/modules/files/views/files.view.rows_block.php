...
//			echo $row['anons'].'</a></p>';
			...
			if(! empty($row["files"]))
			{
				foreach ($row["files"] as $f)
				{
					...
					echo '<a href="'.$f["link"].'" class="download"><strong>'.$row["name"].'</strong></a>';		
					
					echo '<p class="addict-info"><strong>'.$this->diafan->_('Размер').':</strong> '.$f["size"].'</p>';
				}
			}

...
if (! $result["user"])
{
	...
	<span class="new_checkbox"><input type="checkbox" id="not_my_computer" name="not_my_computer" value="1">
	<label for="not_my_computer" class="not_my_computer text-center">'.$this->diafan->_('Чужой компьютер').'</label></span>	

	<br><input type="submit" class="my-3" value="Войти"><br>';
	if (! empty($result["reminding"]))
	{
		echo '<a href="'.$result["reminding"].'" class="arrow-link black mx-2">'.$this->diafan->_('Забыли пароль?').'</a> ';
	}
	if(! empty($result["registration"]))
	{		
		echo '<a href="'.$result["registration"].'" class="arrow-link black mx-2">'.$this->diafan->_('Регистрация').'</a>';
	}
	echo '<div class="err_bot errors error"'.($result["error"] ? '>'.$result["error"] : ' style="display:none">').'</div>';
	echo '</form>';

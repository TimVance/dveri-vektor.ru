...
<div class="errors position-relative shadow-lg c_white b_red fs_16 py-2 mb-5 rounded text-center cur_pointer d-none error"'.($result["error"] ? '>'.$result["error"] : ' style="display:none">').'</div>
...
echo '<div class="border_grey_shad_2 rounded b_white px-4 py-3 mt-5">';
if(! empty($result["payments"]))
{
	echo '<div class="infofield fs_20 mb-4 font-weight-bold text-center">'.$this->diafan->_('Выберите способ оплаты').':</div>';
	echo '<div class="sposob_oplati d-block d-md-flex justify-content-around font-weight-bold mb-4">';
...
echo '
<form method="POST" action="" class="cart_form ajax text-center" enctype="multipart/form-data">
<input type="hidden" name="module" value="cart">
...
if (! empty($result["rows_param"]))
{
	foreach ($result["rows_param"] as $row)
	{
		...
		<div class="errors position-static shadow-none b_red c_white rounded fs_13 error_p'.$row["id"].'"'.($result["error_p".$row["id"]] ? '>'.$result["error_p".$row["id"]] : ' style="display:none">').'</div></div>';
	}
...
echo '<input type="submit" class="fs_20" value="'.$this->diafan->_('Оформить заказ', false).'">';
...
if($result["show_auth"])
{
	echo '<div class="cart_autorization b_white border_grey_shad_2 px-3 mx-auto mt-5">';
